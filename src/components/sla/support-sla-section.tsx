...
/**
 * Qualidade do suporte na tela /sla?tipo=chamado (antes em /suporte/sla): cumprimento de 1ª resposta e de
...
export async function SupportSlaSection({ month, isAdmin }: { month?: string; isAdmin: boolean }) {
  ...
  return (
    <section aria-label="Qualidade do suporte" className="flex flex-col">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <h2 className="text-lg font-semibold">Suporte · {formatCompetence(report.month)}</h2>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <StatCard
          ...
          value={overall.responsePct !== undefined ? formatPercent(overall.responsePct) : "—"}
          icon={<MessageSquareReply />}
          tone={complianceTone(overall.responsePct, targets.slaResponse)}
          hint={`${overall.responseMet}/${overall.responseTotal} · meta ${formatPercent(targets.slaResponse)}`}
        />
        <StatCard
          compact
          label="SLA de solução"
          value={overall.resolutionPct !== undefined ? formatPercent(overall.resolutionPct) : "—"}
          icon={<CheckCircle2 />}
          tone={complianceTone(overall.resolutionPct, targets.slaResolution)}
          hint={`${overall.resolutionMet}/${overall.resolutionTotal} · meta ${formatPercent(targets.slaResolution)}`}
        />
        ...
        <StatCard
          compact
          label="Em risco / violados agora"
          value={formatNumber(report.critical.length)}
          icon={<Timer />}
          tone={report.critical.length > 0 ? "danger" : "success"}
          href="/suporte/chamados?sla=violado"
        />
      </div>
        ...
        <Card>
          ...
          <CardContent className="px-0 pt-0">
            {report.rules.length === 0 ? (
              <EmptyState
                size="sm"
                title="Nenhuma regra de suporte cadastrada"
                description="Cadastre as regras suporte.critico, suporte.alto, suporte.medio e suporte.baixo em Administração > Configurações."
              />
            ) : (
      ...
      <section id="csat" className="scroll-mt-20">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
          <StatCard
            ...
            value={csat.team.average !== undefined ? csat.team.average.toFixed(1).replace(".", ",") : "—"}
            icon={<Smile />}
            tone={csatTone(csat.team.average, csat.target)}
            hint={`meta ${csat.target.toFixed(1).replace(".", ",")} (escala 0–10)`}
          />
          <StatCard compact label="Avaliações" value={formatNumber(csat.team.count)} icon={<MessageSquareReply />} tone="neutral" />
          <StatCard
            compact
            label="Notas 9–10"
            value={formatNumber(csat.team.promoters)}
            icon={<ThumbsUp />}
            tone="success"
            hint={csat.team.count ? formatPercent(csat.team.promoters / csat.team.count) : undefined}
          />
          <StatCard
            compact
            label="Notas 0–6"
            value={formatNumber(csat.team.detractors)}
            icon={<ThumbsDown />}
            tone={csat.team.detractors > 0 ? "danger" : "success"}
            hint="geram tarefa de investigação"
          />
        </div>
                      ...
                      <Link href={`/suporte/chamados/${r.ticketId}`} className="flex gap-3 px-5 py-2.5 hover:bg-surface-hover">
                        <span
                          className={cn(
                            ...
                            r.score >= 9 ? "bg-success-soft text-success-fg" : r.score >= 7 ? "bg-warning-soft text-warning-fg" : "bg-danger-soft text-danger-fg",
                          )}
                        >
                          {r.score}
                        </span>
